...
Pass your own icons for each step state. `half` falls back to `empty` when omitted.
...
| Name  | Type                                                 | Default             | Description            |
| ----- | ---------------------------------------------------- | ------------------- | ---------------------- |
| icons | `{full: IconProp; half?: IconProp; empty: IconProp}` | built-in star/heart | Custom icons per step. |
